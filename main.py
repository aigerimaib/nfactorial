...
def show_intro_screen(game, controller):
    intro_screen = pygame.image.load('data/screens/background.png')
...
def show_death_screen(game, controller):
...
def run_game(game, controller):
    # load level data

    board = Board('data/world.txt')
    gate_location = (285, 128)
    plate_locations = [(337, 168*1.57), (390, 168)]
    gate = Gates(gate_location, plate_locations)
    gates = [gate]

    fire_door_location = (230, 48)
    fire_door = FireDoor(fire_door_location)
    water_door_location = (500, 112)
    ...
    while True:
        ...
        if hydro_girl.is_dead() or magma_boy.is_dead():
            show_death_screen(game, controller)
